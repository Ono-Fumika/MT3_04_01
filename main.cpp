...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (isRotate) {
			...
			// 等速円運動の速度
			ball.velocity = {
				-radius * float(M_PI) * std::sin(angle),
				radius * float(M_PI) * std::cos(angle),
				0
			};
			// 等速円運動の加速度
			ball.acceleration = {
				-(float(M_PI) * float(M_PI)) * (ball.position.x - center.x),
				-(float(M_PI) * float(M_PI)) * (ball.position.y - center.y),
				-(float(M_PI) * float(M_PI)) * (ball.position.z - center.z),
			};
		}
